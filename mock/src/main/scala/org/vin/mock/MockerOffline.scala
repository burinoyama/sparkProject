...
import org.vin.commerce.bean.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
import org.vin.mock.util.{RandomDate, RandomNum, RandomOptions}
...
object MockerOffline {
  ...
  //TODO why can not rerun this method unless deleting the table???
      ...
      while (!isQuit) {
        ...
        if (action == "quit") {
          ...
          action match {
            ...
            case "click" => clickCategoryId = RandomNum(1, cargoNum)
              clickProductId = RandomNum(1, productNum)
            case "order" => orderCategoryIds = RandomNum.multi(1, cargoNum, RandomNum(1, 5), ",", false)
              orderProductIds = RandomNum.multi(1, cargoNum, RandomNum(1, 5), ",", false)
            case "pay" => payCategoryIds = RandomNum.multi(1, cargoNum, RandomNum(1, 5), ",", false)
              payProductIds = RandomNum.multi(1, cargoNum, RandomNum(1, 5), ",", false)
          }
