...
object AreaCountApp {
  ...
  def count(sparkSession: SparkSession): Unit = {
    ...
    val aggr_count = "select area, click_product_id as pid, count(*) as click_count, city_ratio" +
...
//    sparkSession.sql(aggr_count).show(100)
    ...
    // 开窗函数作为一个值
    val sort_in_group = "select area, pid, click_count from " +
    ...
    val join_product = "select tbf.area, pi.product_name, tbf.click_count from tb_before_finished tbf inner join product_info pi where tbf.pid = pi.product_id"
